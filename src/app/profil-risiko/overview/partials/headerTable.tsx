 ...
 const dataHeader = [
  {
   ...
   children: [
    ...
    {
     indikator: "Peningkatan Produktivitas Padi KSPP Kalimantan Tengah (%)",
     target: "10%",
     periodePemantauan: "2025",
    },
   ],
   ...
   >
    <TableHead>
     <TableRow>
      ...
      <TableCell sx={{ bgcolor: grey[300] }}>Target</TableCell>
      <TableCell sx={{ bgcolor: grey[300], borderTopRightRadius: 20 }}>
       Periode Pemantauan
      </TableCell>
     </TableRow>
    </TableHead>
    <TableBody>
     {dataHeader.map((row: any, index: any) => (
      <React.Fragment key={index}>
       <TableRow>
        <TableCell rowSpan={row.children.length}>{row.sasaran}</TableCell>
        <TableCell>{row.children[0].indikator}</TableCell>
        <TableCell>{row.children[0].target}</TableCell>
        <TableCell>{row.children[0].periodePemantauan}</TableCell>
       </TableRow>
       {row.children.slice(1).map((child: any, childIndex: any) => (
        <TableRow key={childIndex}>
         <TableCell>{child.indikator}</TableCell>
         <TableCell>{child.target}</TableCell>
         <TableCell>{child.periodePemantauan}</TableCell>
        </TableRow>
       ))}
      </React.Fragment>
     ))}
