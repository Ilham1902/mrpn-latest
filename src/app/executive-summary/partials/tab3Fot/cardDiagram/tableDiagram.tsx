  ...
  <Box bgcolor={alpha(blue[300], 0.3)} py={2}>
   ...
   >
    ...
    <Box>
     ...
     <Typography fontSize={14} align="center">
      {data.map((x) => x.type == "ST" && <li>{x.value}</li>)}
     </Typography>
    </Box>
    <Box>
     <Typography fontSize={14} fontWeight={600} align="center">
     ...
     <Typography fontSize={14} align="center">
      {data.map((x) => x.type == "WT" && <li>{x.value}</li>)}
     </Typography>
    </Box>
   </Stack>
  </Box>
...
export default function TableDiagram({
 ...
 function generateLocation() {
  const result: string[] = data.lokasi.reduce<string[]>(
   (a, b) => [...a, b.name],
   []
  );
  return result.join(", ");
 ...
 function generateSumberDana() {
  const result: string[] = data.sumber_pendanaan.reduce<string[]>(
   (a, b) => [...a, b.name],
   []
  );
  return result.join(", ");
 ...
 function generateKLD() {
  const result: string[] = data.stakeholder.reduce<string[]>(
   (a, b) => [...a, b.value],
   []
  );
  return result.join(", ");
